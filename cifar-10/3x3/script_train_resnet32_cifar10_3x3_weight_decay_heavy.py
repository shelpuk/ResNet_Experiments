...
with graph.as_default():
    ...
    decay_period = 10000
    learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, decay_period, 0.1, staircase=True)
...
autosave_file = "autosave-resnet32-cifar10-3x3-weight-decay-heavy.ckpt"
statistics_file = "autosave-resnet32-cifar10-3x3-weight-decay-heavy.csv"
  ...
  for step in range(offset, offset + num_steps):
    ...
    if step % cv_period_check == 0:
        train_loss, train_accuracy = get_loss_accuracy(examples=train_images,
                                                       labels=train_labels,
                                                       num_examples_to_test=num_examples_to_test,
